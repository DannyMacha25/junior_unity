...
namespace Valkyrie_VR
{
    public class mesh_pcloud_rendering : MonoBehaviour
    {
        ...
        public PointCloudManager.ColoringMethods coloringMethod = PointCloudManager.ColoringMethods.ZAxisColored;
        public float ymax = 2f, ymin = 1f;
        ...
        public bool new_cloud = false;
        public void subCallback(Messages.sensor_msgs.PointCloud2 msg)
        {
          

            time_of_last_msg = (int)msg.header.stamp.data.sec;
            new_cloud = true;
            ...
            uint offset_rgb = 0;
            if (fieldOffset.ContainsKey("rgb"))
            ...
            pointNum = 0;
            //print("---------------------------------------------------------------------------------------------------------------");
            //This part auto sets the max and min for the color, I dont know how expensive this can get -Danny
            if (include_color && (coloringMethod == PointCloudManager.ColoringMethods.YAxisColored || coloringMethod == PointCloudManager.ColoringMethods.ZAxisColored || coloringMethod == PointCloudManager.ColoringMethods.XAxisColored))
            {
                
                for (int i = 0; i < msg.data.Length; i += (int)msg.point_step)
                {
                
                    uint offset_x_temp = fieldOffset["x"];
                    uint offset_y_temp = fieldOffset["y"];
                    uint offset_z_temp = fieldOffset["z"];

                    float ros_x = ToFloat(msg.data[i + offset_x + 0], msg.data[i + offset_x + 1], msg.data[i + offset_x + 2], msg.data[i + offset_x + 3]);
                    float ros_y = ToFloat(msg.data[i + offset_y + 0], msg.data[i + offset_y + 1], msg.data[i + offset_y + 2], msg.data[i + offset_y + 3]);// + 1.2F;
                    float ros_z = ToFloat(msg.data[i + offset_z + 0], msg.data[i + offset_z + 1], msg.data[i + offset_z + 2], msg.data[i + offset_z + 3]);

                    emVector3 point_emPosition = new emVector3(ros_x, ros_y, ros_z);
                    emTransform point_emTF = new emTransform(new emQuaternion(), point_emPosition);
                    Vector3 unity_position = point_emTF.UnityPosition.Value;
                    Vector3 world_position = rot * Vector3.Scale(unity_position, localScale) + pos;


                    switch (coloringMethod)
                    {
                        case PointCloudManager.ColoringMethods.YAxisColored:
                            if (i == 0)
                            {
                                ymin = world_position.y;
                                ymax = world_position.y;
                            }
                            else
                            {
                                if (world_position.y < ymin)
                                {
                                    ymin = world_position.y;
                                }

                                if (world_position.y > ymax)
                                {
                                    ymax = world_position.y;
                                }
                            }
                            break;
                        case PointCloudManager.ColoringMethods.XAxisColored:
                            if (i == 0)
                            {
                                ymin = world_position.x;
                                ymax = world_position.x;
                            }
                            else
                            {
                                if (world_position.x < ymin)
                                {
                                    ymin = world_position.x;
                                }

                                if (world_position.x > ymax)
                                {
                                    ymax = world_position.x;
                                }
                            }
                            break;
                        case PointCloudManager.ColoringMethods.ZAxisColored:
                            if (i == 0)
                            {
                                ymin = world_position.z;
                                ymax = world_position.z;
                            }
                            else
                            {
                                if (world_position.z < ymin)
                                {
                                    ymin = world_position.z;
                                }

                                if (world_position.z > ymax)
                                {
                                    ymax = world_position.z;
                                }
                            }
            ...
            for (int i = 0; i < msg.data.Length; i += (int)msg.point_step)
            {
                ...
                float ros_y = ToFloat(msg.data[i + offset_y + 0], msg.data[i + offset_y + 1], msg.data[i + offset_y + 2], msg.data[i + offset_y + 3]);// + 1.2F;
                float ros_z = ToFloat(msg.data[i + offset_z + 0], msg.data[i + offset_z + 1], msg.data[i + offset_z + 2], msg.data[i + offset_z + 3]);
                float b = 0;//TEMP SOLUTION THIS COLOR CHECK IS A MESS - Greg
                float g = 0;
                float r = 1;

                if (fieldOffset.ContainsKey("rgb"))
                {
                    b = ToInt(0, 0, 0, msg.data[i + offset_rgb + 0]) * 0.00392156862F;
                    g = ToInt(0, 0, 0, msg.data[i + offset_rgb + 1]) * 0.00392156862F;
                    r = ToInt(0, 0, 0, msg.data[i + offset_rgb + 2]) * 0.00392156862F;
                }
                emVector3 point_emPosition = new emVector3(ros_x, ros_y, ros_z);
                emTransform point_emTF = new emTransform(new emQuaternion(), point_emPosition);
                Vector3 unity_position = point_emTF.UnityPosition.Value;
                Vector3 world_position = rot * Vector3.Scale(unity_position, localScale) + pos;
                if (coloringMethod == PointCloudManager.ColoringMethods.XAxisColored)
                {
                    if (world_position.x < ymin || world_position.x > ymax)
                    {
                        r = 0; g = 0; b = 0;
                    }
                    else
                    {
                        r = 1 - (world_position.x - ymin) / (ymax - ymin);
                        g = 0;
                        b = (world_position.x - ymin) / (ymax - ymin);
                    }
                }
                if (coloringMethod == PointCloudManager.ColoringMethods.YAxisColored)
                {
                    if (world_position.y < ymin || world_position.y > ymax)
                    {
                        r = 0; g = 0; b = 0;
                    }
                    else
                    {
                        r = 1 - (world_position.y - ymin) / (ymax - ymin);
                        g = 0;
                        b = (world_position.y - ymin) / (ymax - ymin);
                    }
                }
                if (coloringMethod == PointCloudManager.ColoringMethods.ZAxisColored)
                {
                    if (world_position.z < ymin || world_position.z > ymax)
                    {
                        r = 0; g = 0; b = 0;
                    }
                    else
                    {
                        r = 1 - (world_position.z - ymin) / (ymax - ymin);
                        g = 0;
                        b = (world_position.z - ymin) / (ymax - ymin);
                    }
                }
                if (coloringMethod == PointCloudManager.ColoringMethods.DistanceFromRobot)
                {
                    float magnitude = unity_position.magnitude;
                    if (magnitude > ymax || magnitude < ymin)
                    {
                        r = 0; g = 0; b = 0;
                    }
                    else
                    {
                        r = 1 - (magnitude - ymin) / (ymax - ymin);
                        g = 0;
                        b = (magnitude - ymin) / (ymax - ymin);
                    }
                }
                Color c = new Color(r, g, b, 1);
                addPoint(unity_position.x, unity_position.y, unity_position.z, c);
            }
            //print("---------------------------------------------------------------------------------------------------------------");
        ...
        void Start()
        {
            ...
            if (is_static)
            {
                ...
                //theCloudObject.transform.position = gameObject.transform.position;
                ...
                for (int i = 11; i < Points + 11; i++)
                {
                    try
                    {
                        //NOTE: The transform correction below was not tested. Different mesh files have different coordiante systems so we should account for this in the future - Greg
                        string[] thisPoint = pointLocations[i].Split(' ');
                        float ros_x = float.Parse(thisPoint[0]);
                        float ros_y = float.Parse(thisPoint[1]);
                        float ros_z = float.Parse(thisPoint[2]);
                        Color c;
                        if (thisPoint[3][0] != 'n')
                        {
            ...
            {
                //ROS.ROS_HOSTNAME = "10.0.4.31";
                //ROS.ROS_MASTER_URI = "http://10.0.4.32:11311";
                ...
                sub = nh.subscribe<Messages.sensor_msgs.PointCloud2>(topic_name, 1, subCallback);
